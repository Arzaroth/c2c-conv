...
function pick(id) {
    ...
    if (!found)
        throw new Error(`c2c: #${id} is missing from the page`);
    return found;
}
const el = {
    mode: pick('mode'),
    modeText: pick('mode-text'),
    link: pick('link'),
    who: pick('who'),
    ...
    historyList: pick('history-list'),
    historyToggle: pick('history-toggle'),
};
...
function refreshKeypad() {
    ...
    for (const button of el.keypad.querySelectorAll('button'))
...
function fit() {
    ...
    if (el.screen.hidden || availableWidth <= 0 || availableHeight <= 0)
        return;
...
function handle(msg) {
    switch (msg.type) {
        ...
        case 'screen':
            ...
            if (msg.cursor)
                term.write(`\x1b[${msg.cursor.y + 1};${msg.cursor.x + 1}H`);
            rescale();
            break;
        case 'named':
            el.who.value = msg.name;
        ...
        case 'control':
            if (!msg.ok)
                el.hint.textContent = `Refused: ${msg.error}.`;
            break;
        case 'policy:mode':
            setMode(msg.mode);
            break;
